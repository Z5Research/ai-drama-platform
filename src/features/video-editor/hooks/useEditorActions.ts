...
export function useEditorActions({ projectId, episodeId }: UseEditorActionsProps) {
  ...
  const saveProject = useCallback(async (project: VideoEditorProject) => {
    const response = await fetch(`/api/editor/projects/${episodeId}`, {
      method: 'PUT',
   ...
   * 发起渲染导出
   */
  const startRender = useCallback(async (editorProjectId: string, options?: {
    format?: 'mp4' | 'webm'
    quality?: 'draft' | 'high' | '720p' | '1080p' | '4K'
  }) => {
    const response = await fetch(`/api/editor/projects/${episodeId}/render`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        editorProjectId,
  ...
  const getRenderStatus = useCallback(async (editorProjectId: string) => {
    const response = await fetch(
      `/api/editor/projects/${episodeId}/render-status?id=${editorProjectId}`
    )
